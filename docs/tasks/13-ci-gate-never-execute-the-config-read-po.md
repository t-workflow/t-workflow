...
- Rebased onto `main` (picked up `#23`, unrelated hygiene work that added a `shellcheck` job to this repository's own CI and fixed pre-existing warnings elsewhere in `tests/test.sh`). Before rebasing, that new job failed on this PR's head for two reasons of its own making: `ci.sh`'s new `"origin/$BASE_REF"..."$PR_REF"` reads as shellcheck's "A"B"C" pattern (SC2140), and `check`/`protected`/`docs`/`exempt`/`reviewer_model` are invisible to shellcheck across `lib.sh`'s dynamic source path (SC2154) — fixed with a `diff_range` variable and inline disable comments (the codebase's existing convention for this exact false positive).
