...
- Second fix pass (the re-review's one low finding, at the human's ask): the ship step says the re-run call is a no-op unless a completed run at the head is red, and the skipped-run message no longer tells the person who just marked the PR ready that CI runs when it is marked ready.
- Folded in at the human's ask (re-planned to add `.t-workflow/scripts/lib.sh` and `.t-workflow/scripts/gate.sh`): the ship gate lists the latest review's medium and low findings, and the merge question restates them, so confirming means the human saw them. Highs still block; mediums and lows never do.
- Third fix pass (at the human's ask; the human re-graded the reviewer's low finding on line endings as a blocker, since a gate that silently says "none" is a false negative): the section reader ignores carriage returns and trailing spaces on headings, a severity heading is trimmed, `- (none)` is not a finding, and a review whose pending human checks are unknown now blocks the ship gate instead of being printed and passed over — the old system's rule. Also the gate's header comment and a duplicated test assignment.
