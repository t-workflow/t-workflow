...
- Second fix pass (the re-review's one medium, at the human's ask): the reviews-only re-send now carries dismissal restrictions, bypass allowances, block creations, lock branch, and fork syncing too, with a test that plants all of them and asserts they survive.
- Third fix pass (the re-review's one low, at the human's ask): a dismissal restriction is carried whenever the key is present, empty lists included, since GitHub uses an empty one to mean the setting is on with nobody listed.
