...
3. Checks: `pr.files` through `.t-workflow/scripts/protected.sh` (exit 0 = protected)
   ...
   permission, a branch rule — is settled by a read-only call when one can, never by
   the tests' stubs.
4. Severity is what a finding does in the pipeline: a task or initiative that can no
   ...
   is high wherever it lives, a skill sentence included. Blocker or high, never lower,
   for a failed check, an unauthorized removal, a path outside scope, a protected path
   with no `## Plan`. Only blocker and high hold the verdict; medium and low are posted
