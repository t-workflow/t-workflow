...
Otherwise the output tells you the trunk, the base (`base:` — the trunk, or for a
child of an initiative its integration branch `wip/<parent>-integration`, which the
gate created from the trunk if it was absent), the branch to reuse or create, whether
the tree is dirty, and the mode (`normal` or `fix`).

## 2. Branch

- `branch: none` → `git fetch origin && git checkout -b <branch> origin/<base>`.
- An existing branch → `git checkout <branch>`; if `branch-behind-base` is above 0,
  `git rebase origin/<base>`. A conflict stops the task: leave the rebase for the
  human, report it.
...
protected path — otherwise `/t-review <id>` for a cold read or `/t-ship <id>`. For a
child of an initiative, say that `/t-ship <id>` merges into the integration branch
on the mechanical gate alone; the human's gate is the initiative's own PR.
