...
An issue already closed as not planned with an open `wip/<id>-revert` PR is a second
pass: skip to step 5 and merge that PR; steps 1 to 4 are done, and step 4 would close
the very PR this pass came to merge.
...
5. A child already merged into its parent's integration branch (its PR is merged, its
   ...
   second pass above and merges it. The issue is already closed (step 3), so CI reads
