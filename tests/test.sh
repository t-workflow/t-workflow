...
out=$(g ship 30); rc=$?; [ "$rc" -eq 1 ] && has "$out" 'BLOCKED: the integration branch conflicts with main — merge it directly, resolve, push: git fetch origin && git checkout wip/30-integration && git merge origin/main && git push origin wip/30-integration$' && ! has "$out" 'child task' && ok || bad "gate ship: a conflicting integration branch names the direct merge, never a child task (exit $rc): $out"
...
(git fetch -q origin && git checkout -q wip/30-integration && git merge -q origin/main && git push -q origin wip/30-integration) || bad "gate ship: the named merge"
pr 102 '[30] Init' OPEN _ _ wip/30-integration main '["docs/tasks/31-a.md","docs/tasks/32-b.md","src/a.txt","src/b.txt"]'
